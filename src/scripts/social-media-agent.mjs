...
// OmniRoute (local) is tried first — it routes to the best available model.
// If OmniRoute is down (e.g. in GitHub Actions), we fall back to direct API calls.
const OMNIROUTE_URL = process.env.OMNIROUTE_URL || 'http://localhost:20128';
const AI_PROVIDERS = [
  {
    name: 'OmniRoute',
    baseUrl: `${OMNIROUTE_URL}/v1/chat/completions`,
    defaultModel: 'auto/best-free',
    apiKey: 'omniroute',  // OmniRoute doesn't need a real key for local access
  },
  {
    name: 'OpenAI',
...
const HAS_ANY_AI_KEY = true; // OmniRoute is always "available" locally
